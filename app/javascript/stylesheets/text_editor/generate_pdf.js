        ...
        .then((result) => {
          ...
          for (let i = 0; i < pdf_ary.length; i++) {
            ...
            if (pdf_ary[i].querySelectorAll("img").length != 0) {
              for (
                let j = 0;
                j < pdf_ary[i].querySelectorAll("img").length;
                j++
              ) {
                let height =
                  pdf_ary[i].querySelectorAll("img")[j].scrollHeight * 0.6;
                let weight =
                  pdf_ary[i].querySelectorAll("img")[j].scrollWidth * 0.6;
                doc.addImage(
                  pdf_ary[i].querySelectorAll("img")[j],
                  "JPEG",
                  30,
                  h,
                  weight,
              ...
              if (pdf_ary[i].tagName == "H1") {
                ...
                let txt = pdf_ary[i].textContent.replace(
                  /(\d)(\w)/g,
                  "\n$1   $2"
                );
                doc.setFontSize(12);
          ...
          if (lang && hljs.getLanguage(lang)) {
            try {
              ...
              let html = lines
                .map((item, index) => {
                  return (
                    '<div><span class="line-num line-index"  data-line="">' +
                    (index + 1) +
                    "</span>" +
                    item +
                    "</div>"
